...
// ---------------- Rod Safety Calculation ----------------
const E = 2.1e5; // N/mm2

function computeRodSafety() {
  const stroke = parseFloat(document.getElementById('rodSafetyStroke')?.value) || 0;
  const endK = parseFloat(document.getElementById('rodEndCondition')?.value) || 1.0;
  const rodDia = parseFloat(document.getElementById('isoRod')?.value) || 0;
  const load_kN = parseFloat(document.getElementById('calcPerCyl')?.textContent) || 0;

  const statusDiv = document.getElementById('rodSafetyStatus');
  if (!stroke || !rodDia || !load_kN) {
    statusDiv.textContent = '--';
    statusDiv.className = 'safety-status';
    return;
  }

  const I = (Math.PI * Math.pow(rodDia, 4)) / 64; // mm^4
  const L = stroke; // mm
  const Pcr = (Math.pow(Math.PI, 2) * E * I) / Math.pow(endK * L, 2); // N
...
document.getElementById('applyRodSafety')?.addEventListener('click', computeRodSafety);
