/* ==========================================================
   HYDRAULIC CYLINDER CALCULATOR - v1.3.3 (MERGED FROM v1.2.0)
   © Design Hydraulics 2025 — All Rights Reserved
   ==========================================================
   ✅ Login Password: Hydra@2025
   ✅ Keeps original Hydraulic Calculator UI & logic
   ✅ Updated:
       - Fixed login
       - Find Cylinder Size modal (safety, ISO sizes, logic fix)
       - Export to Excel (Hydraulic_Cylinders sheet)
...
document.getElementById("addCylinderBtn")?.addEventListener("click", () => {
  ...
  tr.querySelector(".editBtn").addEventListener("click", () => {
    ...
    cells.forEach(c => (c.disabled = !c.disabled));
  });
...
document.getElementById("selectAllCheckbox")?.addEventListener("change", e => {
  document.querySelectorAll(".rowSelect").forEach(chk => (chk.checked = e.target.checked));
});
...
const nearest = (list, val) => list.reduce((a, b) => (Math.abs(b - val) < Math.abs(a - val) ? b : a));
...
applyFindBtn?.addEventListener("click", () => {
  ...
  if (pressureVal > 0) {
    ...
    const sigma = 20; // assumed safe stress N/mm²
  ...
  if (strokeVal && isoRod && perCylLoad) {
    ...
    if (SF < 2) safetyHTML = `<p style="color:red">Rod may buckle (SF=${round(SF,2)})</p>`;
    else if (SF < 3) safetyHTML = `<p style="color:orange">Borderline safe (SF=${round(SF,2)})</p>`;
    else safetyHTML = `<p style="color:green">Rod is safe (SF=${round(SF,2)})</p>`;
  }
  ...
  const targets = selected.length > 0 ? Array.from(selected).map(c => c.closest("tr")) : [tbody.querySelector("tr")];
  targets.forEach(r => {
...
document.getElementById("exportBtn")?.addEventListener("click", async () => {
  try {
    ...
    rows.forEach((r, i) => {
      ...
      const areaRod = Math.PI * (rod / 2) ** 2;
      const volBore = (areaBore * stroke) / 1e6;
      const volRod = ((areaBore - areaRod) * stroke) / 1e6;

      const boreFlow = timeVal ? volBore / (timeVal / 60) : 0;
      const rodFlow = timeVal ? volRod / (timeVal / 60) : 0;

      const boreForce = pressVal ? pressVal * 1e5 * (areaBore / 1e6) : 0;
      const rodForce = pressVal ? pressVal * 1e5 * ((areaBore - areaRod) / 1e6) : 0;
